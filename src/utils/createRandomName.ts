// The logic of this code was copied from the source code below.
...
function createRandom(minInput: number, maxInput: number): number {
...
/**
 *  createRandomName generates a random name from the list of adjectives and surnames in this package
 *  formatted as "adjective_surname". For example 'focused_turing'. If retry is non-zero, a random
 *  integer between 0 and 10 will be added to the end of the name, e.g `focused_turing3`
 */
export default function createRandomName(retry: number): string {
  // eslint-disable-next-line no-constant-condition
  while (true) {
    const left = Lefts[createRandom(0, Lefts.length)];
    const right = Rights[createRandom(0, Rights.length)];
    let name = `${left}_${right}`;
    if (name === 'boring_wozniak') { /* Steve Wozniak is not boring */
      continue;
    }

    if (retry > 0) {
      name = `${name}${createRandom(0, 10)}`;
    }
